...
class RealmSpecializationParser(object):
    ...
    def run(self):
        ...
        if self.realm['key-properties']:
            self.on_keyproperties_parse(self.realm['key-properties'])
            self._parse_topic(self.realm['key-properties'])
            self.on_keyproperties_parsed(self.realm['key-properties'])

        for p in self.realm['process']:
            self.on_process_parse(p)
            self._parse_topic(p)
            for sp in p['subprocess']:
